export { BOOK_TABLE_COLLECTION_SCHEMA } from './model/book.table.model'
export { BOOK_TABLE_TIME_COLLECTION_SCHEMA } from '../bookTableTime/model/book.table.time.model'
import * as find from './find/index'
import * as create from './create/index'
import * as update from './update/index'
import * as data from './data/index'
import * as staff from './staff/index'
import * as user from './user/index'

export { find, create, update, data, staff, user }
